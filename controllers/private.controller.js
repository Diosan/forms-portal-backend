...
export const create = async (req, res) => {
  ...
  console.log("\n\n\n\n DOING SUBMISSION CREATE \n\n\n\n");
  console.log("passed_data:", req.body);
  
  let new_user = {
      agencyMemberUniqueId: req.body.user.agencyMemberUniqueId,
      agencyName: req.body.user.agencyName,
  ...
  try {
    const private_user = await UserModel.create(new_user);
    ...
    await UserModel.findAndCountAll().then((data) => {
      ...
      console.log(
        "Sucessfully fetched all UserModel.. Last ID is: ",
        last_id
      );
      res.status(201).json({
        outcome: "success",
    ...
    console.log("Error Creating private user In Sequelize return http error 500", error);
    res.status(500).json({
      outcome: "error",
      error: error
    });
  ...
  console.log("\n\n\n\n END OF SUBMISSION CREATE \n\n\n\n");

}

export const oldCreate = async (req, res) => {
...
export const requestConsent = async (req, res) => {
  ...
  let dpp_email = process.env.DPP_EMAIL;
  let transporter = nodemailer.createTransport(mailConfig);
